...
% Calculate the overlap matrices Mmn for a composite group of bands
% 
...
function [Mmn] = OverlapsComposite(state, meshInd, recip, neighbours)

% ------------------------------------------
% Calculate the overlap matrices Mmn, Eq. 25
% ------------------------------------------
state = permute(state, [1 3 2]); numKpts = size(state, 1); numBands = size(state, 2);
numQpts = size(state, 3); numBvecs = size(neighbours.Nearest, 2);
Mmn = multiprod(conj(permute(state(:,:,:,ones(1, numBvecs)), [2 1 3 4])), ...
    reshape(state(:,:,neighbours.Nearest), numKpts, numBands, numQpts, numBvecs));
...
switch recip.Dimension
    ...
    case '2D'
        ...
        if sum(neighbours.WeightInd == b) > 0
            ...
            q = meshInd(:,N);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ds(:,:,:,s));
            ind = ind + 1;
        end
        b = 2; % 2nd nearest neighbour vector, -G1
        if sum(neighbours.WeightInd == b) > 0
        ...
        if sum(neighbours.WeightInd == b) > 0
            s = 1; % Correction for right-hand side of cell, G1
            q = meshInd(1:N-1,N);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ds(:,:,2:N,s));
            s = 3; % Correction for top of cell, G2
            q = meshInd(N,1:N-1);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ds(:,:,2:N,s));
        ...
        if sum(neighbours.WeightInd == b) > 0
            s = 2; % Correction for left-hand side of cell, -G1
            q = meshInd(2:N,1);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ds(:,:,1:N-1,s));
            s = 4; % Correction for bottom of cell, -G2
            q = meshInd(1,2:N);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ds(:,:,1:N-1,s));
    ...
    case '3D'
        ...
        if sum(neighbours.WeightInd == b) > 0
            ...
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ...
                reshape(ds(:,:,:,:,s), numKpts, numBands, N^2));
            ind = ind + 1;
        end
        b = 2; % 2nd nearest neighbour vector, -G1
        if sum(neighbours.WeightInd == b) > 0
        ...
        if sum(neighbours.WeightInd == b) > 0
            ...
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ...
                reshape(ds(:,:,2:N,:,s), numKpts, numBands, N*(N-1)));
            s = 3; % Correction for back of cell, G2
            ...
            q = meshInd(N,N,:);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), de(:,:,:,e));
            ind = ind + 1;
        end
        b = 8; % 8th nearest neighbour vector, -(G1+G2)
        if sum(neighbours.WeightInd == b) > 0
            ...
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ...
                reshape(ds(:,:,1:N-1,:,s), numKpts, numBands, N*(N-1)));
            s = 4; % Correction for front of cell, -G2
        ...
        if sum(neighbours.WeightInd == b) > 0
            ...
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ...
                reshape(ds(:,:,2:N,2:N,s), numKpts, numBands, (N-1)^2));
            s = 3; % Correction for back of cell, G2
            ...
            e = 1; % Correction for back-right edge of cell, G1+G2
            q = meshInd(N,N,1:N-1);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), de(:,:,2:N,e));
            e = 5; % Correction for top-right edge of cell, G1+G3
            q = meshInd(1:N-1,N,N);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), de(:,:,2:N,e));
            e = 9; % Correction for top-back edge of cell, G2+G3
        ...
        if sum(neighbours.WeightInd == b) > 0
            ...
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ...
                reshape(ds(:,:,1:N-1,1:N-1,s), numKpts, numBands, (N-1)^2));
            s = 4; % Correction for front of cell, G2
            ...
            e = 2; % Correction for front-left edge of cell, -(G1+G2)
            q = meshInd(1,1,2:N);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q),[2 1 3])), de(:,:,1:N-1,e));
            e = 6; % Correction for bottom-left edge of cell, -(G1+G3)
            q = meshInd(2:N,1,1);
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q),[2 1 3])), de(:,:,1:N-1,e));
        ...
        if sum(neighbours.WeightInd == b) > 0
            ...
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ...
                reshape(ds(:,:,2:N,1:N-1,s), numKpts, numBands, (N-1)^2));
            s = 3; % Correction for back of cell, G2
        ...
        if sum(neighbours.WeightInd == b) > 0
            ...
            Mmn(:,:,q,ind) = multiprod(conj(permute(state(:,:,q), [2 1 3])), ...
                reshape(ds(:,:,1:N-1,2:N,s), numKpts, numBands, (N-1)^2));
            s = 4; % Correction for front of cell, -G2
...
numBands = size(c,2); numQpts = size(c,3); N = sqrt(numQpts);
cTemp = zeros(prod(D), numBands, numQpts); cTemp(gkiIn,:,:) = c;
cTemp = reshape(cTemp, D(2), D(1), numBands, numQpts);
ds = zeros(D(2), D(1), numBands, N, 4);
...
% Reshape ds
ds = reshape(ds, prod(D), numBands, N, 4); ds = ds(gkiIn,:,:,:);

% --------------------------------------------------------------------------------------------------------
% Translates the Bloch states on each corner of a 2D cell by one reciprocal lattice vector to the opposite
% corner of the cell. s labels the corners.
% --------------------------------------------------------------------------------------------------------
function  [dc] = Corners2d(c, meshInd, D, gkiIn)
numBands = size(c,2); numQpts = size(c,3); N = sqrt(numQpts);
cTemp = zeros(prod(D), numBands, numQpts); cTemp(gkiIn,:,:) = c;
cTemp = reshape(cTemp, D(2), D(1), numBands, numQpts);
dc = zeros(D(2), D(1), numBands, 4);
% ci = 1; G1+G2; Shift the states on the bottom left-hand corner of the cell
% one reciprocal lattice vector to the top-right
dc(1:D(2)-1,1:D(1)-1,:,1) = cTemp(2:D(2),2:D(1),:,meshInd(1,1));
% ci = 2; -(G1+G2); Shift the states on the top right-hand corner of the cell
% one reciprocal lattice vector to the bottom-left
dc(2:D(2),2:D(1),:,2) = cTemp(1:D(2)-1,1:D(1)-1,:,meshInd(N,N));
% ci = 3; G1-G2; Shift the states on the top left-hand corner of the cell
% one reciprocal lattice vector to the bottom-right
dc(2:D(2),1:D(1)-1,:,3) = cTemp(1:D(2)-1,2:D(1),:,meshInd(N,1));
% ci = 4; -(G1-G2); Shift the states on the bottom right-hand corner of the cell
% one reciprocal lattice vector to the top-left
dc(1:D(2)-1,2:D(1),:,4) = cTemp(2:D(2),1:D(1)-1,:,meshInd(1,N));
% Reshape dc
dc = reshape(dc, prod(D), numBands, 4); dc = dc(gkiIn,:,:);
...
numBands = size(c,2); numQpts = size(c,3); N = round(numQpts ^ (1/3));
cTemp = zeros(prod(D), numBands, numQpts); cTemp(gkiIn,:,:) = c;
cTemp = reshape(cTemp, D(2), D(1), D(3), numBands, numQpts);
ds = zeros(D(2), D(1), D(3), numBands, N ^ 2, 6);
% s = 1; G1; Shift the states on the left-hand side of the cell one reciprocal lattice vector to the right
...
% Reshape ds
ds = reshape(ds, prod(D), numBands, N, N, 6); ds = ds(gkiIn,:,:,:,:);

% -----------------------------------------------------------------------------------------------------------
% Translates the Bloch states on each edge of a 3D cell by one reciprocal lattice vector to the opposite edge
% of the cell. s labels the corners.
% -----------------------------------------------------------------------------------------------------------
function  [de] = Edges3d(c, meshInd, D, gkiIn)
numBands = size(c,2); numQpts = size(c,3); N = round(numQpts ^ (1/3));
cTemp = zeros(prod(D), numBands, numQpts); cTemp(gkiIn,:,:) = c;
cTemp = reshape(cTemp, D(2), D(1), D(3), numBands, numQpts);
de = zeros(D(2), D(1), D(3), numBands, N, 12);
% e = 1; G1+G2; Shift the states on the front-left edge of the cell
% one reciprocal lattice vector to the back-right
de(1:D(2)-1,1:D(1)-1,:,:,:,1) = cTemp(2:D(2),2:D(1),:,:,meshInd(1,1,:));
% e = 2; -(G1+G2); Shift the states on the back-right edge of the cell
% one reciprocal lattice vector to the front-left
de(2:D(2),2:D(1),:,:,:,2) = cTemp(1:D(2)-1,1:D(1)-1,:,:,meshInd(N,N,:));
% e = 3; G1-G2; Shift the states on the back-left edge of the cell
% one reciprocal lattice vector to the front-right
de(2:D(2),1:D(1)-1,:,:,:,3) = cTemp(1:D(2)-1,2:D(1),:,:,meshInd(N,1,:));
% e = 4; -(G1-G2); Shift the states on the front-right edge of the cell
% one reciprocal lattice vector to the back-left
de(1:D(2)-1,2:D(1),:,:,:,4) = cTemp(2:D(2),1:D(1)-1,:,:,meshInd(1,N,:));
% e = 5; G1+G3; Shift the states on the bottom-left edge of the cell
% one reciprocal lattice vector to the top-right
de(:,1:D(1)-1,1:D(3)-1,:,:,5) = cTemp(:,2:D(1),2:D(3),:,meshInd(:,1,1));
% e = 6; -(G1+G3); Shift the states on the top-right edge of the cell
% one reciprocal lattice vector to the bottom-left
de(:,2:D(1),2:D(3),:,:,6) = cTemp(:,1:D(1)-1,1:D(3)-1,:,meshInd(:,N,N));
% e = 7; G1-G3; Shift the states on the top-left edge of the cell
% one reciprocal lattice vector to the bottom-right
de(:,1:D(1)-1,2:D(3),:,:,7) = cTemp(:,2:D(1),1:D(3)-1,:,meshInd(:,1,N));
% e = 8; -(G1-G3); Shift the states on the bottom-right edge of the cell
% one reciprocal lattice vector to the top-left
de(:,2:D(1),1:D(3)-1,:,:,8) = cTemp(:,1:D(1)-1,2:D(3),:,meshInd(:,N,1));
% e = 9; G2+G3; Shift the states on the bottom-front edge of the cell
% one reciprocal lattice vector to the top-back
de(1:D(2)-1,:,1:D(3)-1,:,:,9) = cTemp(2:D(2),:,2:D(3),:,meshInd(1,:,1));
% e = 10; -(G2+G3); Shift the states on the top-back edge of the cell
% one reciprocal lattice vector to the bottom-front
de(2:D(2),:,2:D(3),:,:,10) = cTemp(1:D(2)-1,:,1:D(3)-1,:,meshInd(N,:,N));
% e = 11; G2-G3; Shift the states on the top-front edge of the cell
% one reciprocal lattice vector to the bottom-back
de(1:D(2)-1,:,2:D(3),:,:,11) = cTemp(2:D(2),:,1:D(3)-1,:,meshInd(1,:,N));
% e = 12; -(G2-G3); Shift the states on the bottom-back edge of the cell
% one reciprocal lattice vector to the top-front
de(2:D(2),:,1:D(3)-1,:,:,12) = cTemp(1:D(2)-1,:,2:D(3),:,meshInd(N,:,1));
% Reshape de
de = reshape(de, prod(D), numBands, N, 12); de = de(gkiIn,:,:,:);
...
cTemp = zeros(prod(D), numBands, numQpts); cTemp(gkiIn,:,:) = c;
cTemp = reshape(cTemp, D(2), D(1), D(3), numBands, numQpts);
dc = zeros(D(2), D(1), D(3), numBands, 8);
% ci = 1; G1+G2+G3; Shift the states on the bottom-front-left corner of the cell
% one reciprocal lattice vector to the top-back-right
dc(1:D(2)-1,1:D(1)-1,1:D(3)-1,:,1) = cTemp(2:D(2),2:D(1),2:D(3),:,meshInd(1,1,1));
% ci = 2; -(G1+G2+G3); Shift the states on the top-back-right corner of the cell
% one reciprocal lattice vector to the bottom-front-left
dc(2:D(2),2:D(1),2:D(3),:,2) = cTemp(1:D(2)-1,1:D(1)-1,1:D(3)-1,:,meshInd(N,N,N));
% ci = 3; G1+G2-G3; Shift the states on the top-front-left corner of the cell
% one reciprocal lattice vector to the bottom-back-right
dc(1:D(2)-1,1:D(1)-1,2:D(3),:,3) = cTemp(2:D(2),2:D(1),1:D(3)-1,:,meshInd(1,1,N));
% ci = 4; -(G1+G2-G3); Shift the states on the bottom-back-right corner of the cell
% one reciprocal lattice vector to the top-front-left
dc(2:D(2),2:D(1),1:D(3)-1,:,4) = cTemp(1:D(2)-1,1:D(1)-1,2:D(3),:,meshInd(N,N,1));
% ci = 5; G1-G2+G3; Shift the states on the bottom-back-left corner of the cell
% one reciprocal lattice vector to the top-front-right
dc(2:D(2),1:D(1)-1,1:D(3)-1,:,5) = cTemp(1:D(2)-1,2:D(1),2:D(3),:,meshInd(N,1,1));
% ci = 6; -(G1-G2+G3); Shift the states on the top-front-right corner of the cell
% one reciprocal lattice vector to the bottom-back-left
dc(1:D(2)-1,2:D(1),2:D(3),:,6) = cTemp(2:D(2),1:D(1)-1,1:D(3)-1,:,meshInd(1,N,N));
% ci = 7; G1-G2-G3; Shift the states on the top-back-left corner of the cell
% one reciprocal lattice vector to the bottom-front-right
dc(2:D(2),1:D(1)-1,2:D(3),:,7) = cTemp(1:D(2)-1,2:D(1),1:D(3)-1,:,meshInd(N,1,N));
% ci = 8; -(G1-G2-G3); Shift the states on the bottom-front-right corner of the cell
% one reciprocal lattice vector to the top-back-left
dc(1:D(2)-1,2:D(1),1:D(3)-1,:,8) = cTemp(2:D(2),1:D(1)-1,2:D(3),:,meshInd(1,N,1));
% Reshape dc
dc = reshape(dc, prod(D), numBands, 8); dc = dc(gkiIn,:,:);
